...
def init_spixel_grid(args,  b_train=True):
    ...
    torch_spix_idx_tensor = torch.tensor(
                np.tile(spix_idx_tensor, (args['batch_size'], 1, 1, 1)))
...
def upfeat(input, prob, up_h=2, up_w=2):
    ...
    gt_frm_top_left = F.interpolate(feat_pd[:, :, :-2 * h_shift, :-2 * w_shift], size=(H, W),mode='nearest')
    feat_sum = gt_frm_top_left * prob.narrow(1,0,1)

    top = F.interpolate(feat_pd[:, :, :-2 * h_shift, w_shift:-w_shift], size=(H, W), mode='nearest')
    feat_sum += top * prob.narrow(1, 1, 1)

    top_right = F.interpolate(feat_pd[:, :, :-2 * h_shift, 2 * w_shift:], size=(H, W), mode='nearest')
    feat_sum += top_right * prob.narrow(1,2,1)

    left = F.interpolate(feat_pd[:, :, h_shift:-w_shift, :-2 * w_shift], size=(H, W), mode='nearest')
    feat_sum += left * prob.narrow(1, 3, 1)

    center = F.interpolate(input, size=(H, W), mode='nearest')
    feat_sum += center * prob.narrow(1, 4, 1)

    right = F.interpolate(feat_pd[:, :, h_shift:-w_shift, 2 * w_shift:], size=(H, W), mode='nearest')
    feat_sum += right * prob.narrow(1, 5, 1)

    bottom_left = F.interpolate(feat_pd[:, :, 2 * h_shift:, :-2 * w_shift], size=(H, W), mode='nearest')
    feat_sum += bottom_left * prob.narrow(1, 6, 1)

    bottom = F.interpolate(feat_pd[:, :, 2 * h_shift:, w_shift:-w_shift], size=(H, W), mode='nearest')
    feat_sum += bottom * prob.narrow(1, 7, 1)

    bottom_right =  F.interpolate(feat_pd[:, :, 2 * h_shift:, 2 * w_shift:], size=(H, W), mode='nearest')
...
def update_spixl_map (spixl_map_idx_in, assig_map_in):
    ...
    if (id_h == h) and (id_w == w):
        ...
        spixl_map_idx = F.interpolate(spixl_map_idx_in.float(), size=(h,w), mode='nearest')

    assig_max,_ = torch.max(assig_map, dim=1, keepdim= True)
    assignment_ = torch.where(assig_map == assig_max, torch.ones_like(assig_map) ,torch.zeros_like(assig_map) )
    new_spixl_map_ = spixl_map_idx * assignment_ # winner take all
    new_spixl_map = torch.sum(new_spixl_map_,dim=1,keepdim=True).long()
...
def spixlIdx(args, b_train = False):
    ...
    torch_spix_idx_tensor = torch.tensor(
        np.tile(spix_idx_tensor, (args.batch_size, 1, 1, 1)).astype(np.float32) )
